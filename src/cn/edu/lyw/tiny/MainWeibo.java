...
import cn.edu.lyw.tiny.customview.TimelineAdapter;
import cn.edu.lyw.tiny.customview.TimelineListView;
...
import cn.edu.lyw.tiny.util.WeiboUtil;
...
public class MainWeibo extends Activity implements ConstantUtil {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mTab3 = (ImageView) findViewById(R.id.img_tasks);
		...
		taskView = inflater.inflate(R.layout.task_list, null);
	...
	private void showTaskList(){
		...
		new AsyncTask<Void, Void, Void>() {
			...
			@Override
			protected void onPostExecute(Void result) {
				taskView.findViewById(R.id.rlTaskLoading).setVisibility(View.GONE);
				simpleAdapter = new SimpleAdapter(MainWeibo.this, listmap, R.layout.task_list_data,
						new String[]{"title","createTime","sponsorName"}, 
						new int[]{R.id.title,R.id.createTime,R.id.sponsorName});
				datalist.setAdapter(simpleAdapter);
				if(listmap.size()>0){
					taskView.findViewById(R.id.noTask).setVisibility(View.GONE);
					datalist.setOnItemClickListener(new onItemClickListenerImpl());
				}else{
					taskView.findViewById(R.id.noTask).setVisibility(View.VISIBLE);
				}
			}
	...
	//点击任务列表时跳到详情页面
	private class onItemClickListenerImpl implements android.widget.AdapterView.OnItemClickListener{

		@SuppressWarnings("unchecked")
		@Override
		public void onItemClick(AdapterView<?> adapter, View view, int position,
				long id) {
			Map<String, Object> map = (Map<String,Object>)MainWeibo.this.simpleAdapter.getItem(position);
			Intent intent = new Intent(MainWeibo.this, TaskDetail.class);
			intent.putExtra("taskId", Integer.valueOf(map.get("id").toString()));
			//TODO 
			startActivity(intent);
		}
	}
